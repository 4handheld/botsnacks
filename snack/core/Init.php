 ...
 */
    ...
    function setGetStartedPayload($returnparameter="http://return.parameter.com"){
        $collect=array(
...
//    curl_setopt($ch, CURLOPT_,$postfields);
  //  curl_setopt($ch, CURLOPT_HTTPHEADER,$header);
    ...
    function returnSetUpValues(){
        $fields="get_started,persistent_menu,greeting,whitelisted_domains";
        $resp=$this->curlGET("https://graph.facebook.com/v2.6/me/messenger_profile?access_token=".$this->developer_access_token."&fields=".$fields, "", array('Content-Type: application/json'), true);
       return $resp; 
    }
    ...
    function setupBot($verification_text,$getstarted_button_payload,$welcome_text,$whiteListedDomains,$bot_menu){
        $this
        ->setupVerify($verification_text)
        ->setGetStartedPayload($getstarted_button_payload)
        ->setWelcomeText($welcome_text) 
        ->whiteListDomains($whiteListedDomains)
        ->setUpMenu($bot_menu);
    }
    
    function returnSetupBotProfile($getstarted_button_payload,$welcome_text,$whiteListedDomains,$bot_menu){
        $welcome_text=$this->insertUserData($welcome_text);
         $collect=array(
             "account_linking_url"=>"",
             ...
             "persistent_menu"=>array(
                    ...
                    "locale"=>"default",
                    "composer_input_disabled"=>false,
                    "call_to_actions"=>$bot_menu
                    )
                ),
             "get_started"=>array("payload"=>"$getstarted_button_payload"),
             "greeting"=>array(array("locale"=>"default","text"=>"$welcome_text")),
             "payment_settings"=>"",
             "target_audience"=>"",
             "home_url"=>"",    
        );
       $to_json=json_encode($collect);
      return  $this->curl("https://graph.facebook.com/v2.6/me/messenger_profile?access_token=".$this->developer_access_token, $to_json, array('Content-Type: application/json'), true);
