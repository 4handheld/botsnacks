...
//    file_put_contents("debug",$msg);
    ...
    function confirmPostBackAsURL(){
      $url=$this->postback="http://localhost/SwissCollection/cleanser.php";
      $response=$this->curl($url, "", array('Content-Type: application/json'), true);
      $this->curl("https://graph.facebook.com/v2.6/me/messages?access_token=".$this->fb_developer_token, $response, array('Content-Type: application/json'), true);
      return $this;    
    }
    
    function confirmQuickReplyAsURL(){
      $url=$this->quickReply;
      $response=$this->curl($url, "", array('Content-Type: application/json'), true);
      $this->curl("https://graph.facebook.com/v2.6/me/messages?access_token=".$this->fb_developer_token, $response, array('Content-Type: application/json'), true);
      return $this;   
...
//        $update=str_replace($search, $replace, $update);
    ...
    function sendPaymentRequest($payprotoken,$description,$amount,$returnabledata=array()){
        if($this->shouldNotReply()){
            return $this;
        }
        $buyer=$this->userid;
        $returnable=array();
        $returnable['usrid']="$buyer";
        $returnable['paypro']="$payprotoken";
        $returnable['desc']="Purchase Cloth";
        $returnable['amount']="$buyer";
        $returnable['extra']=$returnabledata;
        $returnable=json_encode($returnable);
       echo $data=$this->curl("https://4handheld.com/api/paymanager/",$returnable, array('Content-Type: application/json'), true);
        $this->curl("https://graph.facebook.com/v2.6/me/messages?access_token=".$this->fb_developer_token, $data, array('Content-Type: application/json'), true);
        return $this;
    }
...
//        $this->userText="";
    ...
    function setupBot($setup_command){
        //setup configuration file. Comment this line once you are done setting up your bot
    $bot=$this;
    if($this->userText!=$setup_command){
        //Continue only if its bot command
        return;
    }
    include_once 'setup.php';
    $a=$this->init->setupBot($verification_text, $getstarted_button_payload, $welcome_text, $whiteListedDomains, $bot_menu);
    return $this;
    }
    
    private function shouldNotReply(){
        if(!$this->approval){
            return true;
        }
